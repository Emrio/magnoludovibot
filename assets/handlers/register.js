...
module.exports = (metaquery) => {
    ...
    member.addRole(roles.verified, 'Déclenchement de la commande de vérification')
      ...
      .then(() => {
        member.removeRoles([roles.guest, roles.being_verified], 'Déclenchement de la commande de vérification')
          ...
          .then(() => {
            if (args[2] === 'as') {
              var classNum = args[3] ? args[3].last() : null
              // Seconde
              if (/(2nd[1-7])|(2n[1-7])/gi.test(args[3])) {
                member.addRoles([roles.seconde, guild.roles.find(r => r.name === '2nd-' + classNum)], 'Déclenchement de la commande de vérification')
                  .catch((err) => {
                    console.log("[Registration] (❗️) Erreur lors de l'attribution des roles 'Secondes' et '2nd-" + classNum + "': " + err)
                    channel.send("Erreur lors de l'ajout des roles `Secondes` et `2nd-" + classNum + '`. Veuillez réessayez')
                  })
                  .then(() => {
              ...
              // Première S
                ...
                member.addRoles([roles.premiere, guild.roles.find(r => r.name === '1S-' + classNum)], 'Déclenchement de la commande de vérification')
                  .catch((err) => {
                    console.log("[Registration] (❗️) Erreur lors de l'attribution des roles 'Premières' et '1S-" + classNum + "': " + err)
                    channel.send("Erreur lors de l'ajout des roles `Premières` et `1S-" + classNum + '`. Veuillez réessayez')
                  })
                  .then(() => {
                    channel.send('Enregistrement de ' + member + ' terminé!')
                  })
              // Terminale S
                ...
                member.addRoles([roles.terminales, guild.roles.find(r => r.name === 'TS-' + classNum)], 'Déclenchement de la commande de vérification')
                  .catch((err) => {
                    console.log("[Registration] (❗️) Erreur lors de l'attribution des roles 'Terminales' et 'TS-" + classNum + "': " + err)
                    channel.send("Erreur lors de l'ajout des roles `Terminales` et `TS-" + classNum + '`. Veuillez réessayez')
                  })
                  .then(() => {
                    channel.send('Enregistrement de ' + member + ' terminé!')
                  })
  ...
  } else channel.send('Vous devez spécifier un pseudo')
}
