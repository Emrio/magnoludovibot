...
module.exports = (members, classes) => {
  ...
  members.forEach((member) => {
    ...
    if(classes.indexOf(oldLevel) !== classes.length-1 && oldLevel !== null) {

      var newLevel = classes[ classes.indexOf(oldLevel)+1 ]

      member.removeRole(oldLevel).then(() => {
        member.addRole(newLevel).then(() => {
          ...
          if(member.user.id === "228252119956717570") {
            ...
            member.removeRole(oldClass)
              ...
              .then(() => {
                ...
                var updateClassText = "Bonjour ! Tu passes maintenant en classe de **" + newLevel.name + "**. Merci de choisir ta nouvelle classe en cliquant sur le numéro correspondant (1 = " + newLevel.name + " 1, etc...)\nAttention ! Merci de choisir ta vraie classe car tu ne pourras pas la changer après."
      ...
      //var oldClass = member.roles.filter(role => getAllClasses(member.guild).array().includes(role))

      // Get all roles of that fellow
      // Take the ones that start with 2nd- 1S- TS- or 1L or TL
